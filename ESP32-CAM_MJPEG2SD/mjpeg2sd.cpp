...
uint8_t motionVal = 9; // initial motion sensitivity setting 
...
extern const frameStruct frameData[] = {
  {"QQVGA", 160, 120, 25, 0, 1},
  {"n/a", 0, 0, 0, 0, 1}, 
  {"n/a", 0, 0, 0, 0, 1}, 
  {"HQVGA", 240, 176, 25, 0, 1}, 
  {"QVGA", 320, 240, 25, 1, 1}, 
  {"CIF", 400, 296, 25, 1, 1},
  {"VGA", 640, 480, 15, 2, 1}, 
  {"SVGA", 800, 600, 15, 2, 1}, 
  {"XGA", 1024, 768, 15, 3, 1}, 
  {"SXGA", 1280, 1024, 15, 3, 1}, 
  {"UXGA", 1600, 1200, 15, 3, 1}  
};
...
static bool closeMjpeg() {
  ...
  if (captureTime > minSeconds) { 
    ...
    showDebug("Final SD storage time %u ms", (uint)millis() - cTime); 
    ...
    snprintf(mjpegName, sizeof(mjpegName)-1, "%s_%s_%u_%u.mjpeg", 
      partName, frameData[fsizePtr].frameSizeStr, (uint)lround(actualFPS), (uint)lround(vidDuration/1000));
    SD_MMC.rename(partName, mjpegName);
    showDebug("MJPEG close/rename time %u ms", (uint)millis() - hTime); 
    cTime = millis() - cTime;
    ...
    showInfo("Number of frames: %u", (uint)frameCnt);
    showInfo("Required FPS: %u", (uint)FPS);    
    showInfo("Actual FPS: %0.1f", actualFPS);
    showInfo("File size: %0.2f MB", (float)vidSize / ONEMEG);
    if (frameCnt) {
      showInfo("Average frame length: %u bytes", (uint)vidSize / frameCnt);
      showInfo("Average frame monitoring time: %u ms", (uint)dTimeTot / frameCnt);
      showInfo("Average frame buffering time: %u ms", (uint)fTimeTot / frameCnt);
      showInfo("Average frame storage time: %u ms", (uint)wTimeTot / frameCnt);
    }
    showInfo("Average SD write speed: %u kB/s", (uint)((vidSize / wTimeTot) * 1000) / 1024);
    showInfo("File open / completion times: %u ms / %u ms", (uint)oTime, (uint)cTime);
    showInfo("Busy: %u%%", (uint)std::min(100 * (wTimeTot+fTimeTot+dTimeTot+oTime+cTime) / vidDuration, (uint32_t)100));
    showInfo("Free heap %u kBs, largest free block %u kB", (uint)xPortGetFreeHeapSize()>>10, (uint)heap_caps_get_largest_free_block(MALLOC_CAP_DEFAULT)>>10);
    showInfo("***************************\n");
    return true;
  } else {
    ...
    showInfo("Insufficient capture duration: %u secs", (uint)captureTime);
    return false;
  }
}
...
static boolean processFrame() {
  ...
  if(xSemaphoreTake(frameMutex, 100 / portTICK_PERIOD_MS) == pdPASS)
  {
...
void readSD() {
  ...
  showDebug("SD read time %u ms", (uint)millis() - rTime);
...
size_t* getNextFrame() {
  ...
  showDebug("http send time %u ms", (uint)millis() - hTime);
  ...
  if (buffLen && !stopPlayback) {
    if (!remaining) {
      ...
      showDebug("SD wait time %u ms", (uint)millis()-mTime);
      wTimeTot += millis()-mTime;
      mTime = millis();                                 
      memcpy(SDbuffer, SDbuffer+CLUSTERSIZE*2, readLen); // load new cluster from double buffer
      showDebug("memcpy took %u ms for %u bytes", (uint)millis()-mTime, readLen);
      buffLen = readLen;
    ...
    if (streamOffset+skipOver > buffLen) {
      ...
      skipOver = 0;
      showDebug("frame search time %u ms", (uint)millis()-mTime);
      fTimeTot += millis()-mTime;
      if (boundary) {
        ...
        showDebug("frame timer wait %u ms", (uint)millis()-mTime);
  ...
  if (psramFound()) {
    if (prepSD_MMC(ONELINE)) { 
      ...
      xTaskCreatePinnedToCore(
                    ...
                    "captureTask",      /* name of task. */
                    4096 * 2,   /* Stack size of task */
                    NULL,               /* parameter of the task */
                    5,                  /* priority of the task */
                    &captureHandle,      /* Task handle to keep track of created task */
                    1);                   /* pin task to core 1 */    
      xTaskCreatePinnedToCore(
                    &playbackTask,        /* Task function. */
                    "playback",           /* name of task. */
                    4096*4,              /* Stack size of task */
                    NULL,                 /* parameter of the task */
                    4,                   /* priority of the task */
                    &playbackHandle,      /* Task handle to keep track of created task */
                    1);                     /* pin task to core 1*/ 
	  //vTaskStartScheduler();
      showDebug("Free heap %u kB", xPortGetFreeHeapSize()>>10);
